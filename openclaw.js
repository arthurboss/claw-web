...
// include: /tmp/tmpcsp6pvfz.js
  ...
  (() => {
    ...
    loadPackage({"files": [{"filename": "/ASSETS.ZIP", "start": 0, "end": 5671842}, {"filename": "/clacon.ttf", "start": 5671842, "end": 5746054}, {"filename": "/console02.tga", "start": 5746054, "end": 5808806}], "remote_package_size": 5808806});

  })();

// end include: /tmp/tmpcsp6pvfz.js
...
var ASM_CONSTS = {
  721168: ($0) => { try { const name = UTF8ToString($0); let wavFileName = 'sounds/menu/CLICK.WAV'; if (name.includes('SELECT.WAV') || name.includes('SELECT_MENU_ITEM')) { wavFileName = 'sounds/menu/SELECT.WAV'; } else if (name.includes('CLICK.WAV') || name.includes('CHANGE_MENU_ITEM')) { wavFileName = 'sounds/menu/CLICK.WAV'; } else if (name.includes('MENUBED.WAV') || name.includes('MENUMUSIC')) { wavFileName = 'sounds/menu/MENUBED.WAV'; } fetch(wavFileName) .then(function(response) { if (!response.ok) { throw new Error('Failed to load WAV file: ' + response.status + ' ' + response.statusText); } return response.arrayBuffer(); }) .then(function(arrayBuffer) { const audioContext = window.audioContext; return audioContext.decodeAudioData(arrayBuffer); }) .then(function(audioBuffer) { window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(name, audioBuffer); }) .catch(function(error) { console.error('Error loading WAV file:', error); window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(name, { type: 'oscillator', frequency: 800, duration: 0.5 }); }); return true; } catch (e) { console.error('Error loading sound:', e); return false; } },  
 722354: ($0, $1) => { try { const name = UTF8ToString($0); const volume = $1; const soundBuffers = window.soundBuffers; if (!soundBuffers || !soundBuffers.has(name)) { return false; } const soundData = soundBuffers.get(name); const audioContext = window.audioContext; if (soundData.type === 'oscillator') { const oscillator = audioContext.createOscillator(); const gainNode = audioContext.createGain(); oscillator.frequency.setValueAtTime(soundData.frequency, audioContext.currentTime); oscillator.type = 'sine'; gainNode.gain.setValueAtTime(volume * window.soundVolume, audioContext.currentTime); gainNode.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + soundData.duration); oscillator.connect(gainNode); gainNode.connect(audioContext.destination); oscillator.start(); oscillator.stop(audioContext.currentTime + soundData.duration); return true; } else { const source = audioContext.createBufferSource(); const gainNode = audioContext.createGain(); source.buffer = soundData; source.loop = false; gainNode.gain.value = volume * window.soundVolume; source.connect(gainNode); gainNode.connect(audioContext.destination); source.start(); return true; } } catch (e) { console.error('Error playing sound:', e); return false; } },  
 723577: ($0) => { if (window.activeSources && window.activeSources.has(UTF8ToString($0))) { const name = UTF8ToString($0); try { window.activeSources.get(name).stop(); window.activeSources.delete(name); } catch(e) { } } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopSound', name: UTF8ToString($0) }); } },  
 723905: () => { if (window.activeSources) { window.activeSources.forEach((source, path) => { try { source.stop(); } catch(e) { } }); window.activeSources.clear(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopAllSounds' }); } },  
 724160: ($0, $1, $2) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'loadMusic', name: UTF8ToString($0), data: new Uint8Array(HEAPU8.buffer, $1, $2) }); } },  
 724331: ($0, $1, $2) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'playMusic', name: UTF8ToString($0), looping: $1, volume: $2 }); } },  
 724482: () => { if (window.musicSource) { window.musicSource.stop(); window.musicSource = null; } if (window.musicGainNode) { window.musicGainNode = null; } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopMusic' }); } },  
 724725: () => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'pauseMusic' }); } },  
 724828: () => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'resumeMusic' }); } },  
 724932: ($0, $1, $2, $3, $4) => { try { const dataPtr = $0; const dataSize = $1; const volume = $2; const loops = $3; const originalPath = UTF8ToString($4); const audioData = new Uint8Array(dataSize); audioData.set(HEAPU8.subarray(dataPtr, dataPtr + dataSize)); const arrayBuffer = audioData.buffer; const header = String.fromCharCode.apply(null, audioData.slice(0, 4)); if (header !== 'RIFF') { console.error('[Audio] Invalid WAV header:', header); return false; } const audioContext = window.audioContext; audioContext.decodeAudioData(arrayBuffer.slice(0)) .then(function(audioBuffer) { window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(originalPath, audioBuffer); const source = audioContext.createBufferSource(); const gainNode = audioContext.createGain(); source.buffer = audioBuffer; source.loop = (loops === -1); let volumeMultiplier = window.soundVolume; let isMusic = false; if (originalPath.includes('MENUBED.WAV') || originalPath.includes('MENUMUSIC')) { volumeMultiplier = window.musicVolume; isMusic = true; } let finalVolume = volume * volumeMultiplier; if (isMusic && !window.musicEnabled) { finalVolume = 0; } finalVolume = Math.max(0.0, Math.min(1.0, finalVolume)); gainNode.gain.value = finalVolume; if (isMusic) { window.musicGainNode = gainNode; window.musicSource = source; window.musicVolume = volumeMultiplier; } window.activeSources = window.activeSources || new Map(); const isLooping = (loops === -1); if (isLooping || isMusic) { if (window.activeSources.has(originalPath)) { try { window.activeSources.get(originalPath).stop(); } catch(e) {} } window.activeSources.set(originalPath, source); } source.onended = function() { if (window.activeSources && window.activeSources.get(originalPath) === source) { window.activeSources.delete(originalPath); } }; source.connect(gainNode); gainNode.connect(audioContext.destination); source.start(); }) .catch(function(error) { console.error('[Audio] Failed to decode:', originalPath, error); }); return true; } catch (e) { console.error('[Audio] Exception:', e); return false; } },  
 726978: ($0) => { window.soundVolume = $0; if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setSoundVolume', volume: $0 }); } },  
 727122: ($0) => { window.musicEnabled = $0; if (!window.musicEnabled) { if (window.musicGainNode) { window.musicGainNode.gain.value = 0; } } else { if (window.musicGainNode) { window.musicGainNode.gain.value = window.musicVolume; } } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setMusicEnabled', enabled: $0 }); } },  
 727459: ($0) => { window.musicVolume = $0; if (window.musicGainNode && window.musicEnabled) { window.musicGainNode.gain.value = $0; } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setMusicVolume', volume: $0 }); } },  
 727694: () => { try { if (!window.audioContext) { window.AudioContext = window.AudioContext || window.webkitAudioContext; if (!window.AudioContext) { console.error('Web Audio API not supported'); return false; } window.audioContext = new AudioContext(); } if (window.audioContext.state === 'suspended') { window.audioContext.resume(); } window.soundVolume = 1.0; window.musicVolume = 1.0; window.musicEnabled = true; window.soundEnabled = true; return true; } catch (e) { console.error('Error initializing AudioContext:', e); return false; } },  
 728224: () => { try { if (!window.audioContext || !window.audioContext.audioWorklet) { console.error('AudioWorklet not supported in this browser'); return false; } const audioWorkletScript = 'class GameAudioProcessor extends AudioWorkletProcessor {' + '    constructor() {' + '        super();' + '        this.soundVolume = 1.0;' + '        this.musicVolume = 1.0;' + '        this.port.onmessage = (event) => { this.handleMessage(event.data); };' + '    }' + '    handleMessage(data) {' + '        switch (data.type) {' + '            case "setSoundVolume": this.soundVolume = data.volume; break;' + '            case "setMusicVolume": this.musicVolume = data.volume; break;' + '        }' + '    }' + '    process(inputs, outputs, parameters) {' + '        // Simple audio processing - just pass through with volume control' + '        const output = outputs[0];' + '        if (output && output.length > 0) {' + '            for (let channel = 0; channel < output.length; channel++) {' + '                const outputChannel = output[channel];' + '                if (outputChannel) {' + '                    for (let i = 0; i < outputChannel.length; i++) {' + '                        outputChannel[i] = 0; // Clear output' + '                    }' + '                }' + '            }' + '        }' + '        return true;' + '    }' + '}' + 'registerProcessor("game-audio-processor", GameAudioProcessor);'; const blob = new Blob([audioWorkletScript], { type: 'application/javascript' }); const url = URL.createObjectURL(blob); return new Promise((resolve, reject) => { window.audioContext.audioWorklet.addModule(url) .then(() => { try { window.audioWorkletNode = new AudioWorkletNode(window.audioContext, 'game-audio-processor'); window.audioWorkletNode.connect(window.audioContext.destination); resolve(true); } catch (e) { console.error('Error creating AudioWorkletNode:', e); reject(e); } }) .catch(error => { console.error('Error loading AudioWorklet module:', error); reject(error); }); }).then(() => true).catch(() => false); } catch (e) { console.error('Error setting up AudioWorklet:', e); return false; } },  
 730337: ($0) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage(JSON.parse(UTF8ToString($0))); } },  
 730446: () => { console.log('[Debug] m_videoPlatform is NULL!'); },  
 730499: () => { var canvas = document.getElementById('canvas'); return canvas ? canvas.width : 640; },  
 730587: () => { var canvas = document.getElementById('canvas'); return canvas ? canvas.height : 480; },  
 730676: ($0, $1) => { if (typeof triggerHaptic === 'function') { triggerHaptic($0, UTF8ToString($1)); } },  
 730762: ($0, $1, $2, $3) => { if (typeof triggerHapticCustom === 'function') { triggerHapticCustom($0, $1, $2, $3); } },  
 730854: ($0) => { if (typeof setHapticEnabled === 'function') { setHapticEnabled($0); } },  
 730928: ($0) => { try { localStorage.setItem('openclaw:saves', UTF8ToString($0)); return 1; } catch (e) { console.error('[SaveBridge] Save error:', e); return 0; } },  
 731078: () => { try { var jsonStr = localStorage.getItem('openclaw:saves'); if (!jsonStr) return 0; var lengthBytes = lengthBytesUTF8(jsonStr) + 1; var ptr = _malloc(lengthBytes); stringToUTF8(jsonStr, ptr, lengthBytes); return ptr; } catch (e) { console.warn('[SaveBridge] Load error:', e); return 0; } },  
 731370: () => { try { return localStorage.getItem('openclaw:saves') ? 1 : 0; } catch (e) { return 0; } },  
 731461: () => { try { localStorage.removeItem('openclaw:saves'); return 1; } catch (e) { console.error('[SaveBridge] Delete error:', e); return 0; } },  
 731598: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.cleanup) { window.webglBridge.cleanup(); } },  
 731713: ($0, $1, $2, $3) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.setViewport) { window.webglBridge.setViewport($0, $1, $2, $3); } },  
 731850: ($0, $1, $2, $3) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.clear) { window.webglBridge.clear($0, $1, $2, $3); } },  
 731975: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.present) { window.webglBridge.present(); } },  
 732090: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.initialize) { return window.webglBridge.initialize() ? 1 : 0; } return 0; },  
 732236: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.createShaders) { return window.webglBridge.createShaders() ? 1 : 0; } return 0; },  
 732388: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.createBuffers) { return window.webglBridge.createBuffers() ? 1 : 0; } return 0; },  
 732540: ($0, $1, $2, $3, $4, $5, $6, $7) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.renderQuad) { window.webglBridge.renderQuad($0, $1, $2, $3, $4, $5, $6, $7); } },  
 732691: () => { if (!window.textureBridge) { window.textureBridge = new TextureBridge(); } window.textureBridge.initialize(); },  
 732805: () => { if (window.textureBridge) { window.textureBridge.shutdown(); delete window.textureBridge; window.textureBridge = null; } },  
 732930: ($0, $1) => { if (window.textureBridge) { return window.textureBridge.loadTexture(UTF8ToString($0), $1) ? 1 : 0; } return 0; },  
 733045: ($0) => { return window.textureBridge ? window.textureBridge.getTextureWidth($0) : 0; },  
 733125: ($0) => { return window.textureBridge ? window.textureBridge.getTextureHeight($0) : 0; },  
 733206: ($0) => { return window.textureBridge ? window.textureBridge.getTextureFormat($0) : 0; },  
 733287: ($0) => { return window.textureBridge ? window.textureBridge.hasTransparency($0) : 0; },  
 733367: ($0) => { if (window.textureBridge) { window.textureBridge.unloadTexture($0); } },  
 733441: () => { window.exportSaveData(); },  
 733470: () => { window.importSaveData(); },  
 733499: () => { window.toggleFullscreen(); },  
 733530: () => { window.toggleAspectRatio(); },  
 733562: () => { toggleFPS(); },  
 733579: () => { return document.fullscreenElement ? 1 : 0; },  
 733626: () => { return document.fullscreenElement ? 0 : 1; },  
 733673: () => { return window.forceOriginalAspect ? 0 : 1; },  
 733720: () => { return window.forceOriginalAspect ? 1 : 0; },  
 733767: () => { return window.fpsEnabled ? 1 : 0; },  
 733805: () => { return window.fpsEnabled ? 0 : 1; },  
 733843: () => { return document.fullscreenElement ? 1 : 0; },  
 733890: () => { return document.fullscreenElement ? 0 : 1; },  
 733937: () => { return window.forceOriginalAspect ? 0 : 1; },  
 733984: () => { return window.forceOriginalAspect ? 1 : 0; },  
 734031: () => { return window.fpsEnabled ? 1 : 0; },  
 734069: () => { return window.fpsEnabled ? 0 : 1; },  
 734107: ($0) => { console.log('[Gamepad C++] Controller ' + $0 + ' connected'); },  
 734173: ($0) => { console.log('[Gamepad C++] Controller ' + $0 + ' disconnected'); },  
 734242: () => { return window.devicePixelRatio ? window.devicePixelRatio : 1.0; },  
 734310: ($0) => { var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return reply.length === 1 ? reply.charCodeAt(0) : -1; },  
 734525: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 734672: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
 734906: ($0) => { if(typeof(Module['SDL2']) === 'undefined') { Module['SDL2'] = {}; } var SDL2 = Module['SDL2']; if (!$0) { SDL2.audio = {}; } else { SDL2.capture = {}; } if (!SDL2.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL2.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL2.audioContext = new webkitAudioContext(); } if (SDL2.audioContext) { if ((typeof navigator.userActivation) === 'undefined') { autoResumeAudioContext(SDL2.audioContext); } } } return SDL2.audioContext === undefined ? -1 : 0; },  
 735458: () => { var SDL2 = Module['SDL2']; return SDL2.audioContext.sampleRate; },  
 735526: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; var have_microphone = function(stream) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); SDL2.capture.silenceTimer = undefined; SDL2.capture.silenceBuffer = undefined } SDL2.capture.mediaStreamNode = SDL2.audioContext.createMediaStreamSource(stream); SDL2.capture.scriptProcessorNode = SDL2.audioContext.createScriptProcessor($1, $0, 1); SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) { if ((SDL2 === undefined) || (SDL2.capture === undefined)) { return; } audioProcessingEvent.outputBuffer.getChannelData(0).fill(0.0); SDL2.capture.currentCaptureBuffer = audioProcessingEvent.inputBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.mediaStreamNode.connect(SDL2.capture.scriptProcessorNode); SDL2.capture.scriptProcessorNode.connect(SDL2.audioContext.destination); SDL2.capture.stream = stream; }; var no_microphone = function(error) { }; SDL2.capture.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.capture.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { SDL2.capture.currentCaptureBuffer = SDL2.capture.silenceBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); if ((navigator.mediaDevices !== undefined) && (navigator.mediaDevices.getUserMedia !== undefined)) { navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(have_microphone).catch(no_microphone); } else if (navigator.webkitGetUserMedia !== undefined) { navigator.webkitGetUserMedia({ audio: true, video: false }, have_microphone, no_microphone); } },  
 737219: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; SDL2.audio.scriptProcessorNode = SDL2.audioContext['createScriptProcessor']($1, 0, $0); SDL2.audio.scriptProcessorNode['onaudioprocess'] = function (e) { if ((SDL2 === undefined) || (SDL2.audio === undefined)) { return; } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); SDL2.audio.silenceTimer = undefined; SDL2.audio.silenceBuffer = undefined; } SDL2.audio.currentOutputBuffer = e['outputBuffer']; dynCall('vp', $2, [$3]); }; SDL2.audio.scriptProcessorNode['connect'](SDL2.audioContext['destination']); if (SDL2.audioContext.state === 'suspended') { SDL2.audio.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.audio.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { if ((typeof navigator.userActivation) !== 'undefined') { if (navigator.userActivation.hasBeenActive) { SDL2.audioContext.resume(); } } SDL2.audio.currentOutputBuffer = SDL2.audio.silenceBuffer; dynCall('vp', $2, [$3]); SDL2.audio.currentOutputBuffer = undefined; }; SDL2.audio.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); } },  
 738394: ($0, $1) => { var SDL2 = Module['SDL2']; var numChannels = SDL2.capture.currentCaptureBuffer.numberOfChannels; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.capture.currentCaptureBuffer.getChannelData(c); if (channelData.length != $1) { throw 'Web Audio capture buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } if (numChannels == 1) { for (var j = 0; j < $1; ++j) { setValue($0 + (j * 4), channelData[j], 'float'); } } else { for (var j = 0; j < $1; ++j) { setValue($0 + (((j * numChannels) + c) * 4), channelData[j], 'float'); } } } },  
 738999: ($0, $1) => { var SDL2 = Module['SDL2']; var buf = $0 >>> 2; var numChannels = SDL2.audio.currentOutputBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.audio.currentOutputBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[buf + (j*numChannels + c)]; } } },  
 739488: ($0) => { var SDL2 = Module['SDL2']; if ($0) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); } if (SDL2.capture.stream !== undefined) { var tracks = SDL2.capture.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL2.capture.stream.removeTrack(tracks[i]); } } if (SDL2.capture.scriptProcessorNode !== undefined) { SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL2.capture.scriptProcessorNode.disconnect(); } if (SDL2.capture.mediaStreamNode !== undefined) { SDL2.capture.mediaStreamNode.disconnect(); } SDL2.capture = undefined; } else { if (SDL2.audio.scriptProcessorNode != undefined) { SDL2.audio.scriptProcessorNode.disconnect(); } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); } SDL2.audio = undefined; } if ((SDL2.audioContext !== undefined) && (SDL2.audio === undefined) && (SDL2.capture === undefined)) { SDL2.audioContext.close(); SDL2.audioContext = undefined; } },  
 740494: ($0, $1, $2) => { var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Browser.createContext(Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); SDL2.data32Data = data; } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); },  
 741960: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 742948: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 743031: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 743100: () => { return window.innerWidth; },  
 743130: () => { return window.innerHeight; }
};
