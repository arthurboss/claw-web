...
// include: /tmp/tmp1yfzfvyi.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = '/home/testuser/projects/dealer-poc-pwa/Build_Release/openclaw.data';
    ...
    for (var file of metadata['files']) {
      ...
      async function processPackageData(arrayBuffer) {
        ...
        Module['removeRunDependency'](`fp ${name}`);
          ...
          Module['removeRunDependency']('datafile_/home/testuser/projects/dealer-poc-pwa/Build_Release/openclaw.data');
      }
      Module['addRunDependency']('datafile_/home/testuser/projects/dealer-poc-pwa/Build_Release/openclaw.data');
...
// end include: /tmp/tmp1yfzfvyi.js
...
var ASM_CONSTS = {
  721440: ($0) => { try { const name = UTF8ToString($0); let wavFileName = 'sounds/menu/CLICK.WAV'; if (name.includes('SELECT.WAV') || name.includes('SELECT_MENU_ITEM')) { wavFileName = 'sounds/menu/SELECT.WAV'; } else if (name.includes('CLICK.WAV') || name.includes('CHANGE_MENU_ITEM')) { wavFileName = 'sounds/menu/CLICK.WAV'; } else if (name.includes('MENUBED.WAV') || name.includes('MENUMUSIC')) { wavFileName = 'sounds/menu/MENUBED.WAV'; } fetch(wavFileName) .then(function(response) { if (!response.ok) { throw new Error('Failed to load WAV file: ' + response.status + ' ' + response.statusText); } return response.arrayBuffer(); }) .then(function(arrayBuffer) { const audioContext = window.audioContext; return audioContext.decodeAudioData(arrayBuffer); }) .then(function(audioBuffer) { window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(name, audioBuffer); }) .catch(function(error) { console.error('Error loading WAV file:', error); window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(name, { type: 'oscillator', frequency: 800, duration: 0.5 }); }); return true; } catch (e) { console.error('Error loading sound:', e); return false; } },  
 722626: ($0, $1) => { try { const name = UTF8ToString($0); const volume = $1; const soundBuffers = window.soundBuffers; if (!soundBuffers || !soundBuffers.has(name)) { return false; } const soundData = soundBuffers.get(name); const audioContext = window.audioContext; if (soundData.type === 'oscillator') { const oscillator = audioContext.createOscillator(); const gainNode = audioContext.createGain(); oscillator.frequency.setValueAtTime(soundData.frequency, audioContext.currentTime); oscillator.type = 'sine'; gainNode.gain.setValueAtTime(volume * window.soundVolume, audioContext.currentTime); gainNode.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + soundData.duration); oscillator.connect(gainNode); gainNode.connect(audioContext.destination); oscillator.start(); oscillator.stop(audioContext.currentTime + soundData.duration); return true; } else { const source = audioContext.createBufferSource(); const gainNode = audioContext.createGain(); source.buffer = soundData; source.loop = false; gainNode.gain.value = volume * window.soundVolume; source.connect(gainNode); gainNode.connect(audioContext.destination); source.start(); return true; } } catch (e) { console.error('Error playing sound:', e); return false; } },  
 723849: ($0) => { if (window.activeSources && window.activeSources.has(UTF8ToString($0))) { const name = UTF8ToString($0); try { window.activeSources.get(name).stop(); window.activeSources.delete(name); } catch(e) { } } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopSound', name: UTF8ToString($0) }); } },  
 724177: () => { if (window.activeSources) { window.activeSources.forEach((source, path) => { try { source.stop(); } catch(e) { } }); window.activeSources.clear(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopAllSounds' }); } },  
 724432: ($0, $1, $2) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'loadMusic', name: UTF8ToString($0), data: new Uint8Array(HEAPU8.buffer, $1, $2) }); } },  
 724603: ($0, $1, $2) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'playMusic', name: UTF8ToString($0), looping: $1, volume: $2 }); } },  
 724754: () => { if (window.musicSource) { window.musicSource.stop(); window.musicSource = null; } if (window.musicGainNode) { window.musicGainNode = null; } if (window.stopLevelMidi) { window.stopLevelMidi(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopMusic' }); } },  
 725051: () => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'pauseMusic' }); } },  
 725154: () => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'resumeMusic' }); } },  
 725258: ($0, $1, $2, $3, $4) => { try { const dataPtr = $0; const dataSize = $1; const volume = $2; const loops = $3; const originalPath = UTF8ToString($4); const audioData = new Uint8Array(dataSize); audioData.set(HEAPU8.subarray(dataPtr, dataPtr + dataSize)); const arrayBuffer = audioData.buffer; const header = String.fromCharCode.apply(null, audioData.slice(0, 4)); if (header !== 'RIFF') { console.error('[Audio] Invalid WAV header:', header); return false; } const audioContext = window.audioContext; audioContext.decodeAudioData(arrayBuffer.slice(0)) .then(function(audioBuffer) { window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(originalPath, audioBuffer); const source = audioContext.createBufferSource(); const gainNode = audioContext.createGain(); source.buffer = audioBuffer; source.loop = (loops === -1); let volumeMultiplier = window.soundVolume; let isMusic = false; if (originalPath.includes('MENUBED.WAV') || originalPath.includes('MENUMUSIC')) { volumeMultiplier = window.musicVolume; isMusic = true; } let finalVolume = volume * volumeMultiplier; if (isMusic && !window.musicEnabled) { finalVolume = 0; } finalVolume = Math.max(0.0, Math.min(1.0, finalVolume)); gainNode.gain.value = finalVolume; if (isMusic) { if (window.musicSource) { try { window.musicSource.stop(); } catch(e) {} } window.musicGainNode = gainNode; window.musicSource = source; window.musicVolume = volumeMultiplier; } window.activeSources = window.activeSources || new Map(); const isLooping = (loops === -1); if (isLooping && !isMusic) { if (window.activeSources.has(originalPath)) { try { window.activeSources.get(originalPath).stop(); } catch(e) {} } window.activeSources.set(originalPath, source); } source.onended = function() { if (window.activeSources && window.activeSources.get(originalPath) === source) { window.activeSources.delete(originalPath); } }; source.connect(gainNode); gainNode.connect(audioContext.destination); source.start(); }) .catch(function(error) { console.error('[Audio] Failed to decode:', originalPath, error); }); return true; } catch (e) { console.error('[Audio] Exception:', e); return false; } },  
 727380: ($0) => { window.soundVolume = $0; if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setSoundVolume', volume: $0 }); } },  
 727524: ($0) => { window.musicEnabled = $0; if (!window.musicEnabled) { if (window.musicGainNode) { window.musicGainNode.gain.value = 0; } } else { if (window.musicGainNode) { window.musicGainNode.gain.value = window.musicVolume; } } if (window.setLevelMidiVolume) { window.setLevelMidiVolume(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setMusicEnabled', enabled: $0 }); } },  
 727925: ($0) => { window.musicVolume = $0; if (window.musicGainNode && window.musicEnabled) { window.musicGainNode.gain.value = $0; } if (window.setLevelMidiVolume) { window.setLevelMidiVolume(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setMusicVolume', volume: $0 }); } },  
 728224: () => { try { if (!window.audioContext) { window.AudioContext = window.AudioContext || window.webkitAudioContext; if (!window.AudioContext) { console.error('Web Audio API not supported'); return false; } window.audioContext = new AudioContext(); } if (window.audioContext.state === 'suspended') { window.audioContext.resume(); } window.soundVolume = 1.0; window.musicVolume = 1.0; window.musicEnabled = true; window.soundEnabled = true; return true; } catch (e) { console.error('Error initializing AudioContext:', e); return false; } },  
 728754: () => { try { if (!window.audioContext || !window.audioContext.audioWorklet) { console.error('AudioWorklet not supported in this browser'); return false; } const audioWorkletScript = 'class GameAudioProcessor extends AudioWorkletProcessor {' + '    constructor() {' + '        super();' + '        this.soundVolume = 1.0;' + '        this.musicVolume = 1.0;' + '        this.port.onmessage = (event) => { this.handleMessage(event.data); };' + '    }' + '    handleMessage(data) {' + '        switch (data.type) {' + '            case "setSoundVolume": this.soundVolume = data.volume; break;' + '            case "setMusicVolume": this.musicVolume = data.volume; break;' + '        }' + '    }' + '    process(inputs, outputs, parameters) {' + '        // Simple audio processing - just pass through with volume control' + '        const output = outputs[0];' + '        if (output && output.length > 0) {' + '            for (let channel = 0; channel < output.length; channel++) {' + '                const outputChannel = output[channel];' + '                if (outputChannel) {' + '                    for (let i = 0; i < outputChannel.length; i++) {' + '                        outputChannel[i] = 0; // Clear output' + '                    }' + '                }' + '            }' + '        }' + '        return true;' + '    }' + '}' + 'registerProcessor("game-audio-processor", GameAudioProcessor);'; const blob = new Blob([audioWorkletScript], { type: 'application/javascript' }); const url = URL.createObjectURL(blob); return new Promise((resolve, reject) => { window.audioContext.audioWorklet.addModule(url) .then(() => { try { window.audioWorkletNode = new AudioWorkletNode(window.audioContext, 'game-audio-processor'); window.audioWorkletNode.connect(window.audioContext.destination); resolve(true); } catch (e) { console.error('Error creating AudioWorkletNode:', e); reject(e); } }) .catch(error => { console.error('Error loading AudioWorklet module:', error); reject(error); }); }).then(() => true).catch(() => false); } catch (e) { console.error('Error setting up AudioWorklet:', e); return false; } },  
 730867: ($0) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage(JSON.parse(UTF8ToString($0))); } },  
 730976: () => { console.log('[Debug] m_videoPlatform is NULL!'); },  
 731029: () => { var canvas = document.getElementById('canvas'); return canvas ? canvas.width : 640; },  
 731117: () => { var canvas = document.getElementById('canvas'); return canvas ? canvas.height : 480; },  
 731206: ($0, $1) => { if (typeof triggerHaptic === 'function') { triggerHaptic($0, UTF8ToString($1)); } },  
 731292: ($0, $1, $2, $3) => { if (typeof triggerHapticCustom === 'function') { triggerHapticCustom($0, $1, $2, $3); } },  
 731384: ($0) => { if (typeof setHapticEnabled === 'function') { setHapticEnabled($0); } },  
 731458: ($0) => { try { localStorage.setItem('openclaw:saves', UTF8ToString($0)); return 1; } catch (e) { console.error('[SaveBridge] Save error:', e); return 0; } },  
 731608: () => { try { var jsonStr = localStorage.getItem('openclaw:saves'); if (!jsonStr) return 0; var lengthBytes = lengthBytesUTF8(jsonStr) + 1; var ptr = _malloc(lengthBytes); stringToUTF8(jsonStr, ptr, lengthBytes); return ptr; } catch (e) { console.warn('[SaveBridge] Load error:', e); return 0; } },  
 731900: () => { try { return localStorage.getItem('openclaw:saves') ? 1 : 0; } catch (e) { return 0; } },  
 731991: () => { try { localStorage.removeItem('openclaw:saves'); return 1; } catch (e) { console.error('[SaveBridge] Delete error:', e); return 0; } },  
 732128: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.cleanup) { window.webglBridge.cleanup(); } },  
 732243: ($0, $1, $2, $3) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.setViewport) { window.webglBridge.setViewport($0, $1, $2, $3); } },  
 732380: ($0, $1, $2, $3) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.clear) { window.webglBridge.clear($0, $1, $2, $3); } },  
 732505: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.present) { window.webglBridge.present(); } },  
 732620: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.initialize) { return window.webglBridge.initialize() ? 1 : 0; } return 0; },  
 732766: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.createShaders) { return window.webglBridge.createShaders() ? 1 : 0; } return 0; },  
 732918: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.createBuffers) { return window.webglBridge.createBuffers() ? 1 : 0; } return 0; },  
 733070: ($0, $1, $2, $3, $4, $5, $6, $7) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.renderQuad) { window.webglBridge.renderQuad($0, $1, $2, $3, $4, $5, $6, $7); } },  
 733221: () => { if (!window.textureBridge) { window.textureBridge = new TextureBridge(); } window.textureBridge.initialize(); },  
 733335: () => { if (window.textureBridge) { window.textureBridge.shutdown(); delete window.textureBridge; window.textureBridge = null; } },  
 733460: ($0, $1) => { if (window.textureBridge) { return window.textureBridge.loadTexture(UTF8ToString($0), $1) ? 1 : 0; } return 0; },  
 733575: ($0) => { return window.textureBridge ? window.textureBridge.getTextureWidth($0) : 0; },  
 733655: ($0) => { return window.textureBridge ? window.textureBridge.getTextureHeight($0) : 0; },  
 733736: ($0) => { return window.textureBridge ? window.textureBridge.getTextureFormat($0) : 0; },  
 733817: ($0) => { return window.textureBridge ? window.textureBridge.hasTransparency($0) : 0; },  
 733897: ($0) => { if (window.textureBridge) { window.textureBridge.unloadTexture($0); } },  
 733971: ($0, $1) => { if (window.playLevelMidiBuffer) { var bytes = HEAPU8.slice($0, $0 + $1); window.playLevelMidiBuffer(bytes); } },  
 734085: () => { window.exportSaveData(); },  
 734114: () => { window.importSaveData(); },  
 734143: () => { window.toggleFullscreen(); },  
 734174: () => { window.toggleAspectRatio(); },  
 734206: () => { return document.fullscreenElement ? 1 : 0; },  
 734253: () => { return document.fullscreenElement ? 0 : 1; },  
 734300: () => { return window.forceOriginalAspect ? 0 : 1; },  
 734347: () => { return window.forceOriginalAspect ? 1 : 0; },  
 734394: () => { var iOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1); if (iOS) return 0; return document.fullscreenElement ? 1 : 0; },  
 734587: () => { var iOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1); if (iOS) return 0; return document.fullscreenElement ? 0 : 1; },  
 734780: () => { return window.forceOriginalAspect ? 0 : 1; },  
 734827: () => { return window.forceOriginalAspect ? 1 : 0; },  
 734874: ($0) => { console.log('[Gamepad C++] Controller ' + $0 + ' connected'); },  
 734940: ($0) => { console.log('[Gamepad C++] Controller ' + $0 + ' disconnected'); },  
 735009: () => { return window.devicePixelRatio ? window.devicePixelRatio : 1.0; },  
 735077: ($0) => { var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return reply.length === 1 ? reply.charCodeAt(0) : -1; },  
 735292: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 735439: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
